...
class ModelLayer {
  ...
  async createSharedEditor ({sharedBufferId, scrollPosition}) {
    const {id} = await this.db.one(`
      INSERT INTO shared_editors (shared_buffer_id, scroll_position)
      VALUES ($1, $2)
      RETURNING id
    `, [sharedBufferId, scrollPosition])
    return id
  }

  async findSharedEditor (id) {
    const result = await this.db.one(`
      SELECT shared_buffer_id, scroll_position
      FROM shared_editors
      WHERE id = $1
    `, [id])
    return {sharedBufferId: result.shared_buffer_id, scrollPosition: result.scroll_position}
  }
