...
class ModelLayer {
  ...
  async createSharedEditor ({sharedBufferId, selectionRanges}) {
    const {id} = await this.db.one(`
      INSERT INTO shared_editors (shared_buffer_id, selection_ranges)
      VALUES ($1, $2)
      RETURNING id
    `, [sharedBufferId, selectionRanges])
    return id
  }

  setSharedEditorSelectionRanges (sharedEditorId, selectionRanges) {
    return this.db.none(`
      UPDATE shared_editors
      SET selection_ranges = $2
      WHERE id = $1
    `, [sharedEditorId, selectionRanges])
  }

  async findSharedEditor (id) {
    const result = await this.db.one(`
      SELECT shared_buffer_id, selection_ranges
      FROM shared_editors
      WHERE id = $1
    `, [id])
    return {
      sharedBufferId: result.shared_buffer_id,
      selectionRanges: result.selection_ranges
    }
  }
